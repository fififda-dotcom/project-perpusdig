...
.form-group input:focus, .form-group select:focus {
    outline: none;
    border-color: #4A90E2; /* Biru admin focus */
...
.chapter-editor-header .chapter-num-badge {
    background: #ebd5c5;
    color: #4a3c31;
    padding: 4px 10px;
    border-radius: 8px;
...
.chapter-editor-card input[type="text"] {
    margin-bottom: 0px;
}
...
.chapter-editor-card textarea:focus {
    outline: none;
    border-color: #E69C62;
}
...
<div class="paw-container">
    <h1><i class="fas fa-edit" style="color: #4A90E2;"></i> Edit Data Buku</h1>
    <p class="paw-subtitle">Sesuaikan dan ubah rincian data buku pada koleksi PawLib 🐾</p>

    <div class="form-box">
        <form action="/buku/update/<?= $buku['id'] ?><?= !empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '' ?>" method="post" enctype="multipart/form-data">
            <!-- Hidden context inputs for correct redirection -->
            <input type="hidden" name="page" value="<?= isset($_GET['page']) ? esc($_GET['page']) : 1 ?>">
            <input type="hidden" name="ref" value="<?= isset($_GET['ref']) ? esc($_GET['ref']) : '' ?>">
            <input type="hidden" name="rak_id" value="<?= isset($_GET['rak_id']) ? esc($_GET['rak_id']) : '' ?>">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="tahun" value="<?= esc($buku['tahun']) ?>" required>
                </div>

                <div class="form-group">
                    <label>Jenis Koleksi <span class="text-danger">*</span></label>
                    <select name="jenis_koleksi" id="jenis_koleksi" required>
                        <option value="fisik" <?= ($buku['jenis_koleksi'] ?? 'fisik') === 'fisik' ? 'selected' : '' ?>>Buku Fisik</option>
                        <option value="ebook" <?= ($buku['jenis_koleksi'] ?? 'fisik') === 'ebook' ? 'selected' : '' ?>>E-Book</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="nomor_panggil" value="<?= esc($buku['nomor_panggil'] ?? '') ?>" placeholder="Contoh: 813.4 MEW p">
                </div>
                
                <div class="form-group">
                    <label>Nomor Klasifikasi</label>
                    <input type="text" name="nomor_klasifikasi" value="<?= esc($buku['nomor_klasifikasi'] ?? '') ?>" placeholder="Contoh: 813.4">
                </div>
                
                <div class="form-group">
                    <label>ISBN</label>
                    <input type="text" name="isbn" value="<?= esc($buku['isbn'] ?? '') ?>" placeholder="Contoh: 978-602-1234-56-7">
                </div>
                
                <div class="form-group">
                    <label>Dimensi Buku</label>
                    <input type="text" name="dimensi_buku" value="<?= esc($buku['dimensi_buku'] ?? '') ?>" placeholder="Contoh: 15 x 23 cm">
                </div>
                
                <div class="form-group">
                    <label>Departemen / Kategori</label>
                    <input type="text" name="departemen" value="<?= esc($buku['departemen'] ?? '') ?>" placeholder="Contoh: Fiksi, Komputer, Jurnal">
                </div>
                
                <div class="form-group">
                    <label>Edisi Buku</label>
                    <input type="text" name="edisi" value="<?= esc($buku['edisi'] ?? '') ?>" placeholder="Contoh: Edisi Pertama, Cetakan Ke-2">
                </div>
                
                <div class="form-group">
                    <label>Bahasa</label>
                    <?php 
                        $savedLang = $buku['bahasa'] ?? 'Indonesia'; 
                        $standardLangs = ['Indonesia', 'Inggris', 'Arab', 'Mandarin', 'Jepang', 'Korea', 'Jerman', 'Prancis'];
                        $isCustom = !in_array($savedLang, $standardLangs) && !empty($savedLang) && $savedLang !== 'Lainnya';
                    ?>
                    <select name="bahasa" id="bahasa_select" onchange="toggleCustomLanguageInput(this, 'custom_bahasa_container', 'custom_bahasa_input')">
                        <option value="Indonesia" <?= $savedLang === 'Indonesia' ? 'selected' : '' ?>>Indonesia</option>
                        <option value="Inggris" <?= $savedLang === 'Inggris' ? 'selected' : '' ?>>Inggris</option>
                        <option value="Arab" <?= $savedLang === 'Arab' ? 'selected' : '' ?>>Arab</option>
                        <option value="Mandarin" <?= $savedLang === 'Mandarin' ? 'selected' : '' ?>>Mandarin</option>
                        <option value="Jepang" <?= $savedLang === 'Jepang' ? 'selected' : '' ?>>Jepang</option>
                        <option value="Korea" <?= $savedLang === 'Korea' ? 'selected' : '' ?>>Korea</option>
                        <option value="Jerman" <?= $savedLang === 'Jerman' ? 'selected' : '' ?>>Jerman</option>
                        <option value="Prancis" <?= $savedLang === 'Prancis' ? 'selected' : '' ?>>Prancis</option>
                        <?php if ($isCustom): ?>
                            <option value="<?= esc($savedLang) ?>" selected><?= esc($savedLang) ?></option>
                        <?php endif; ?>
                        <option value="Lainnya" <?= $savedLang === 'Lainnya' ? 'selected' : '' ?>>Lainnya...</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="tempat_terbit" value="<?= esc($buku['tempat_terbit'] ?? '') ?>" placeholder="Contoh: Jakarta, Bandung">
                </div>

                <div class="form-group">
                    <label>Deskripsi Fisik (Ilustrasi, dll.)</label>
                    <input type="text" name="deskripsi_fisik" value="<?= esc($buku['deskripsi_fisik'] ?? '') ?>" placeholder="Contoh: ilus. , il.">
                </div>
            </div>
            ...
            <div class="form-group" style="margin-bottom: 15px;">
                ...
                <textarea name="sinopsis" style="padding: 14px 18px; border: 2px solid #ebd5c5; border-radius: 12px; font-family: 'Quicksand', sans-serif; font-size: 0.95rem; font-weight: 600; color: #4a3c31; background-color: #ffffff; transition: all 0.3s ease; width: 100%; box-sizing: border-box; min-height: 100px; resize: vertical;" placeholder="Tulis sinopsis atau deskripsi buku di sini..."><?= esc($buku['sinopsis'] ?? '') ?></textarea>
            </div>

            <div class="form-group" style="margin-bottom: 25px;">
                <label>Catatan Tambahan (Notes Area)</label>
                <textarea name="catatan" style="padding: 14px 18px; border: 2px solid #ebd5c5; border-radius: 12px; font-family: 'Quicksand', sans-serif; font-size: 0.95rem; font-weight: 600; color: #4a3c31; background-color: #ffffff; transition: all 0.3s ease; width: 100%; box-sizing: border-box; min-height: 80px; resize: vertical;" placeholder="Tulis catatan bibliografi atau keterangan tambahan di sini..."><?= esc($buku['catatan'] ?? '') ?></textarea>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" name="stok" value="<?= esc($buku['stok']) ?>" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="rak_id" required>
                        ...
                        ?>
                            <option value="<?= $r['id'] ?>" <?= (isset($buku['rak_id']) && $buku['rak_id'] == $r['id']) ? 'selected' : '' ?>>
            ...
            <?php
                $db = \Config\Database::connect();
                $existingChapters = [];
                if (($buku['jenis_koleksi'] ?? 'fisik') === 'ebook') {
                    $existingChapters = $db->table('ebook_bab')
            ...
            ?>
            <div id="section_ebook" style="display: <?= ($buku['jenis_koleksi'] ?? 'fisik') === 'ebook' ? 'block' : 'none' ?>;">
                <div class="form-section-title">
                    <i class="fas fa-file-pdf"></i> Konten Digital E-Book
                </div>
                <div class="form-grid">
                    <div class="form-group">
                        <label>Upload File E-Book (PDF atau EPUB - Biarkan kosong jika tidak ingin mengganti berkas)</label>
                        ...
                        <?php if (!empty($buku['file_pdf'])): ?>
                            <p style="font-size: 0.85rem; color: #8c7b70; margin-top: 5px;">
                                File PDF aktif saat ini: <strong><a href="/uploads/ebooks/<?= esc($buku['file_pdf']) ?>" target="_blank"><?= esc($buku['file_pdf']) ?></a></strong>
                            </p>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label>Jumlah Halaman E-Book</label>
                        <input type="number" name="jumlah_halaman" id="jumlah_halaman" placeholder="Contoh: 150" value="<?= esc($buku['jumlah_halaman'] ?? 0) ?>">
                    </div>
                    <div class="form-group">
            ...
            <div class="form-group">
                <label>Ganti Cover (Biarkan kosong jika tidak ingin mengubah cover)</label>
                ...
                <?php 
                    ...
                    if(!empty($coverField)): 
                        $path = file_exists(FCPATH . 'uploads/cover/' . $coverField) ? '/uploads/cover/' . $coverField : '/uploads/' . $coverField;
                ?>
                    <div class="cover-preview-box">
                        <img src="<?= $path ?>" class="cover-preview-img" alt="Cover Lama">
                        <div>
                            <span class="d-block text-muted" style="font-size: 0.8rem;">Cover aktif saat ini:</span>
                            <strong style="font-size: 0.85rem; word-break: break-all;"><?= esc($coverField) ?></strong>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="form-actions">
                <button type="submit" class="btn-submit" onclick="return confirm('Rak PawLib siap diperbarui. Simpan perubahan?')">
                    <i class="fas fa-save"></i> Perbarui Data Buku
                </button>
                <?php
                    $batalUrl = '/buku';
                    if (isset($_GET['ref']) && $_GET['ref'] === 'rak_detail') {
                        $batalUrl = '/rak/detail/' . (isset($_GET['rak_id']) ? esc($_GET['rak_id']) : '');
                    } elseif (isset($_GET['page'])) {
                        $batalUrl = '/buku?page_buku=' . esc($_GET['page']);
                    }
                ?>
                <a href="<?= $batalUrl ?>" class="btn-cancel">
...
let chapters = <?= json_encode($existingChapters) ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    // Auto toggle on load
    const isEbook = document.getElementById('jenis_koleksi').value === 'ebook';
    ...
    if (isEbook) {
        ...
        if (chapters && chapters.length > 0) {
            ...
            modeBacaCheckbox.checked = false;
...
// PDF & EPUB Parsing & Structured Text Extraction (only if new file uploaded)
...
// Check on load if custom language is select's value to adjust visibility
document.addEventListener('DOMContentLoaded', function() {
    const selectEl = document.getElementById('bahasa_select');
    if (selectEl && selectEl.value === 'Lainnya') {
        document.getElementById('custom_bahasa_container').style.display = 'block';
        document.getElementById('custom_bahasa_input').setAttribute('required', 'required');
    }
});
